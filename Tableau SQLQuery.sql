...
From CovidDeaths
where continent is not null and continent like '%frica%' and total_cases is not null
Group By location
order by 1,2
...
Select location, SUM(new_cases) as total_cases, SUM(cast(new_deaths as int)) as total_deaths, SUM(cast(new_deaths as int))/SUM(New_Cases)*100 as DeathPercentage
From CovidDeaths
where continent is not null and continent like '%urope%' and total_cases is not null
Group By location
order by 1,2
...
Select location, SUM(new_cases) as total_cases, SUM(cast(new_deaths as int)) as total_deaths, SUM(cast(new_deaths as int))/SUM(New_Cases)*100 as DeathPercentage
From CovidDeaths
where continent is not null and continent like '%merica%' and total_cases is not null
Group By location
order by 1,2
